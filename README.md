...
поступающих точек плоскости и вычисляющую её периметр, площадь и 
периметр части, которая лежит внутри заданного замкнутого треугольника. 
Треугольник задаётся тремя разными точками плоскости. Решение
...
Был создан класс `Triangle`, который содержит вершины заданного треугольника, 
а также методы, с помощью которых вычисляется периметр части оболочки, лежащей внутри треугольника. 
Методы `border` и `is_border` определяют, принадлежит ли точка и отрезок границе треугольника соответсвенно. 
Методы `inside` и `is_inside` определяют, принадлежит ли точка и отрезок внутренности треугольника соответсвенно. 
Методы `is_equal_1`, `..._2`, `..._3` определяют, совпадает ли точка с вершиной треугольника. 
Метод `is_intersect` определяет, пересекаются ли два отрезка. 
Методы `intersection1, ...2, ...3` возвращают точку пересечения стороны оболочки со стороной треугольника. 
Метод `length` возвращает длину отрезка. 
Метод `is_convex` определяет, проходит ли сторона оболочки через вершину треугольника. 
Метод `inside_length` возвращает длину части стороны оболочки, которая лежит внутри треугольника. 
Метод `cross` класса `R2Point` возвращает модуль векторного произведения двух векторов на плоскости. 
Метод `g` калассов `Polygon` и `Segment` отвечает за вычисление периметра части оболочки, лежащей внутри треугольника.
