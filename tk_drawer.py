...
class TkDrawer:
    ...
    def draw_triangle(self, p, q, g):
        self.canvas.create_line(x(p), y(p), x(q), y(q), fill="purple", width=2)
        self.canvas.create_line(x(p), y(p), x(g), y(g), fill="green", width=2)
        self.canvas.create_line(x(g), y(g), x(q), y(q), fill="orange", width=2)
        self.root.update()
